(function (global, factory) {
    ...
    */
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_1 + ".render() method has side effects on the state of " + vm + "." + propName);
                    assert.invariant(!isUpdatingTemplate, "When updating the template of " + vmBeingRendered_1 + ", one of the accessors used by the template has side effects on the state of " + vm + "." + propName);
    ...
    BaseLightningElementConstructor.prototype = {
        ...
        addEventListener: function (type, listener, options) {
            ...
            if (process.env.NODE_ENV !== 'production') {
                ...
                assert.invariant(!isInvokingRender, vmBeingRendered_2 + ".render() method has side effects on the state of " + vm + " by adding an event listener for \"" + type + "\".");
                assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_2 + " has side effects on the state of " + vm + " by adding an event listener for \"" + type + "\".");
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_3 + ".render() method has side effects on the state of " + vm + "." + toString(key));
                    assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_3 + " has side effects on the state of " + vm + "." + toString(key));
                }
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_4 + ".render() method has side effects on the state of " + vm + "." + toString(key));
                    assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_4 + " has side effects on the state of " + vm + "." + toString(key));
                }
    ...
    function createConnector(vm, name, wireDef) {
        var method = wireDef.method, adapter = wireDef.adapter, configCallback = wireDef.configCallback, params = wireDef.params;
        var hasDynamicParams = params.length > 0;
        var component = vm.component;
    ...
    function storeWiredMethodMeta(descriptor, adapter, configCallback, params) {
    ...
    function storeWiredFieldMeta(descriptor, adapter, configCallback, params) {
        // support for callable adapters
        if (adapter.adapter) {
            adapter = adapter.adapter;
        }
    ...
    function registerDecorators(Ctor, meta) {
        ...
        if (!isUndefined(wire)) {
            for (var fieldOrMethodName in wire) {
                var _b = wire[fieldOrMethodName], adapter = _b.adapter, method = _b.method, configCallback = _b.config, _c = _b.params, params = _c === void 0 ? {} : _c;
    ...
    function ti(value) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            if (shouldNormalize) {
                logError("Invalid tabindex value `" + toString(value) + "` in template for " + vmBeingRendered_6 + ". This attribute must be set to 0 or -1.", vmBeingRendered_6);
            }
        }
    ...
    var startMeasure = !isUserTimingSupported ? noop$2 : function (phase, vm) {
        var markName = getMarkName(phase, vm);
        start(markName);
    };
    var endMeasure = !isUserTimingSupported ? noop$2 : function (phase, vm) {
        var markName = getMarkName(phase, vm);
    ...
    function markComponentAsDirty(vm) {
        if (process.env.NODE_ENV !== 'production') {
            ...
            assert.isFalse(isInvokingRender, "markComponentAsDirty() for " + vm + " cannot be called during rendering of " + vmBeingRendered_7 + ".");
            assert.isFalse(isUpdatingTemplate, "markComponentAsDirty() for " + vm + " cannot be called while updating template of " + vmBeingRendered_7 + ".");
        }
        vm.isDirty = true;
    }
